...
from graph_nav.model.graph.GraphAction import GraphAction
from graph_nav.model.graph.GraphState import GraphState
from scipy.spatial import distance


class GraphWorld(IWorld):
    def __init__(self, networkx_graph, human_path, time_limit=1000):
        ...
        for s in self.states:
            ...
            for a in self.actions[s]:
                ...
                for s_prim in self.states_prim[s][a]:
                    R[s][a][s_prim] = self.graph.nodes[s_prim.robot_node]['weight'] - self.graph[s.robot_node][s_prim.robot_node]['weight'] - self.euclidean_distance_between_node(s.robot_node, s_prim.robot_node) - self.euclidean_distance_between_node(s_prim.robot_node, self.global_goal)
                    if s_prim.robot_node == s_prim.human_node or s.human_node == s_prim.robot_node:
                        R[s][a][s_prim] -= 100
        return R
