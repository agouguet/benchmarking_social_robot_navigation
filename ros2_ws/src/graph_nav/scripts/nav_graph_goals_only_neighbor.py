...
from unity_sim.msg import GraphNav
from unity_sim.msg import SceneInfo
...
from graph_nav.model.graph_boolean_with_goals.GraphWorldBooleanWithGoal import GraphWorldBooleanWithGoal
from graph_nav.model.graph_boolean_with_goals_only_neighbor.GraphWorldBooleanWithGoalOnlyNeighbor import GraphWorldBooleanWithGoalOnlyNeighbor
...
PATH_TO_SAVE_POLICY = "/home/adam/ws/my_unity_sim/ros2_ws/src/unity_sim/policies/"
EXTENSION_FILE = ".pickle"

DISTANCE_LIMIT_OCCUPIED_NODE = 0.8
DISTANCE_LIMIT_ROBOT_NEAREST_NODE = 0.75
TIME_OCCUPANCY = 10
...
class GraphNavNode(Node):
    ...
    def graph_callback(self, msg_graph):
        if self.graph == None and self.scenario != None:
            ...
            self.problem = GraphWorldBooleanWithGoalOnlyNeighbor(self.graph)
            # print(self.problem.actions[GraphStateBooleanWithGoalOnlyNeighbor(2, (), 12)])
            ...
            if os.path.isfile(PATH_TO_SAVE_POLICY + self.scenario + EXTENSION_FILE):
                ...
                self.solver = ValueIteration(self.problem, gamma=0.5)
                self.solver.train()
                self.policy = self.solver.policy
                print("Save policy as ...", PATH_TO_SAVE_POLICY + self.scenario + EXTENSION_FILE)
                pickle.dump(self.policy, open(PATH_TO_SAVE_POLICY + self.scenario + EXTENSION_FILE, 'wb'))
                print("Save policy full values as ...", PATH_TO_SAVE_POLICY + self.scenario + "_values" + EXTENSION_FILE)
                pickle.dump(self.solver.full_values, open(PATH_TO_SAVE_POLICY + self.scenario + "_values" + EXTENSION_FILE, 'wb'))
    ...
    def robot_odom_callback(self, msg_odom):
        if self.robot_goal != None and self.policy != None:
            robot_position = msg_odom.pose.pose.position
            self.robot_state = self.get_current_state_robot(robot_position)
            humans_state = self.get_current_state_humans([pos[0] for pos in self.humans_position])
            humans_traj = self.get_current_state_humans([traj[0] for traj in self.human_trajectories])
            occupied_node = humans_state + humans_traj
            occupied_node = tuple(sorted(set([i for i in occupied_node if i in [n for n in (list(self.graph.neighbors(self.robot_state))+[self.robot_state])]]))) # remove duplicates, sort and keep only neighbors
            print("OCUUPIED:", occupied_node, self.graph.neighbors(self.robot_state))
            current_state = (self.robot_state, occupied_node, self.robot_goal)

            if(current_state != self.current_state):
                print("NEW STATE:", self.current_state, "->", current_state, "      ", type(self.current_state))
                self.current_state = current_state
                action = self.policy[GraphStateBooleanWithGoalOnlyNeighbor(self.robot_state, occupied_node, self.robot_goal)]
