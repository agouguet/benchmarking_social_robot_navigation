...
from graph_nav.utils.visibility_graph import VisibilityGraph
...
PATH_TO_MAP = "/home/adam/ws/my_unity_sim/ros2_ws/src/unity_sim/maps/"
...
class GraphNavNode(Node):
    ...
    def graph_callback(self, msg_graph):
        if self.graph == None and self.scenario != None:
            ...
            path_policies_values = PATH_TO_SAVE_POLICY + self.scenario + "_values" + EXTENSION_FILE
            ...
            if os.path.isfile(path_policies_values):
                ...
                visibility_graph = VisibilityGraph(PATH_TO_MAP + self.scenario + "/", self.graph)

                self.problem = GraphWorldBooleanWithGoal(self.graph, visibility_graph.visibility_graph, numbers_human = 3, debug_mode=True)
                ...
                if self.save_policy:
                    ...
                    print("Save astar dict as ...", PATH_TO_SAVE_POLICY + self.scenario + "_astar" + EXTENSION_FILE)
                    pickle.dump(self.problem.astar_dict, open(PATH_TO_SAVE_POLICY + self.scenario + "_astar" + EXTENSION_FILE, 'wb'))
    ...
    def robot_odom_callback(self, msg_odom):
        if self.robot_goal != None:
            ...
            if(current_state != self.current_state):
                ...
                action_values = self.policy[GraphStateBooleanWithGoal(self.robot_state, occupied_node, self.robot_goal)]
                action = max(action_values, key=action_values.get) #self.policy[GraphStateBooleanWithGoal(self.robot_state, occupied_node, self.robot_goal)]
...
def main(args=None):
    ...
    minimal_publisher = GraphNavNode(save_policy=True)
