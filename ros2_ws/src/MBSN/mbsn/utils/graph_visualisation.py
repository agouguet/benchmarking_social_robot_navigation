...
class GraphVisualisation():
    ...
    def node_label(self, node):
        return "V(%s) =\n%0.2f\\nN = %d" % (node.state, node.get_value()[1], node.get_visits())

    def single_agent_node_to_graph(self, g, node, level):
        g.node(str(node.id), label=self.node_label(node), **{'width':str(1.5), 'height':str(1.5), 'fixedsize':str(True), 'fontsize':str(8)})
    ...
    def environment_node(self, g, node, action, level):
        ...
        for (child, probability) in node.children[action]:
            ...
            g.edge(node_id, str(child.id), str("%0.2f" % probability))
