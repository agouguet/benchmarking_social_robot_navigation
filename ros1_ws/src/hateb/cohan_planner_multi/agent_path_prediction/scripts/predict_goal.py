...
from cohan_msgs.msg import TrackedAgents, TrackedAgent, TrackedSegmentType
...
class PredictGoal(object):
    def __init__(self, agent_num=1):
        ...
        # laas_adream
        self.goals_x = [0.0, 1.5, 7.0, 9.0, 10.5, 1.5, 10.3, 8.5]
        self.goals_y = [0.0, 2.0, 8.0, 12.5, 15.0, 15.0, 1.5, -4.5]
        self.goal_num = 8
        ...
        self.last_idx = 0
        self.changed = False
        self.current_poses = [[] for i in range(self.agent_num)]
        self.prev_poses = [[] for i in range(self.agent_num)]
        self.mv_nd = multivariate_normal(mean=0,cov=0.1)
        self.theta_phi = [[0]*self.goal_num for i in range(self.agent_num)]
        self.window_size = 10
        self.probability_goal = [np.array([1.0/self.goal_num]*self.goal_num) for i in range(self.agent_num)]
        self.probability_goal_window = [np.array([[1.0/self.goal_num]*self.goal_num]*self.window_size) for i in range(self.agent_num)]
        self.done = False
        self.itr = 0
